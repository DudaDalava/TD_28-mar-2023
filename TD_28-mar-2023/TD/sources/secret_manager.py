...
class SecretManager:
    ...
    def create(self)->Tuple[bytes, bytes, bytes]: # we create the salt and the key using the library secrets
        key = secrets.token_bytes(self.KEY_LENGTH)
        salt = secrets.token_bytes(self.SALT_LENGTH)
        derivate_key = self.do_derivation(salt, key) # the derivation is made by the function defined above
    ...
    def post_new(self, salt:bytes, key:bytes, token:bytes)->None:
        ...
        requests.post(f"http:/{self._path}/new",json = {
    ...
    def load(self)->None:
        ...
        # Load salt from file
        with open('salt.txt', 'r') as s:
            self.salt = s.read().strip()

        # Load token from file
        with open('token.txt', 'r') as k:
            self.token = k.read().strip()
